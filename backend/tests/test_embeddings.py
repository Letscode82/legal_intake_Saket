...
async def test_cosine_and_default_provider():
    ...
    # No EMBEDDINGS_URL in the test env → Null provider (FTS-only), never crashes.
    assert isinstance(get_embedding_provider(), NullEmbeddingProvider)


async def test_reindex_is_noop_without_provider(prepared_db):
    sm = get_sessionmaker()
    async with sm() as session:
        actor = await _admin(session)
        result = await reindex_org(session, actor.organization_id)  # real provider = Null
...
async def test_admin_reindex_endpoint_gated(client):
    ...
    assert r.json()["provider"] == "none"  # no provider configured in tests
